...
using Flux, MLDatasets, ProgressMeter, MLUtils, CUDA, Dates
using BSON: @save
...
_train_data = [(train_X,train_Y)] |> gpu
...
# Bring model back for inference
model = model |> cpu
...
@save "improved_gpu-$(Dates.now())-$(epochs)-$(a_sum_/length(test_Y)).bson" model
